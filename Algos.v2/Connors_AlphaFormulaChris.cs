...
namespace TuringTrader.BooksAndPubs
{
    ...
    public class Connors_AlphaFormula_RisingAssetsChris : Algorithm
    {
        public override string Name => "Connors' Rising Assets";
        private Plotter _plotter;
        public Connors_AlphaFormula_RisingAssetsChris()
        {
        ...
        public virtual double CalculateMomentum(Instrument i) =>
        ...
        public override IEnumerable<Bar> Run(DateTime? startTime, DateTime? endTime)
        {
            ...
            var universeDs = AddDataSources(UNIVERSE);
            var benchmarkDs = AddDataSource(BENCHMARK);

            //========== simulation loop ==========

            foreach (var s in SimTimes)
            {
                if (!HasInstruments(universeDs) || !HasInstrument(benchmarkDs))
                    continue;

                // calculate momentum and volatility
                var instrumentIndicators = universeDs
                    .ToDictionary(
                        d => d.Instrument,
                        d => (
                            Momentum: CalculateMomentum(d.Instrument),
                            Volatility: d.Instrument.Close.Volatility(63)[0]));

                // take the top 5 assets with the highest momentum
                var top5Lookup = instrumentIndicators
                    .OrderByDescending(kv => kv.Value.Momentum)
                    .Take(5)
                    .Select(kv => (Instrument: kv.Key, Indicators: kv.Value))
                    .ToDictionary(kv => kv.Instrument, kv => kv.Indicators);
                ...
                if (SimTime[0].Month != NextSimTime.Month)
                {
                    ...
                    var sumInverseVol = top5Lookup
                        .Sum(kv => 1.0 / Math.Max(1e-10, kv.Value.Volatility));

                    foreach (var instrument in universeDs.Select(ds => ds.Instrument))
                    {
                        double weight = top5Lookup.ContainsKey(instrument)
                            ? 1.0 / top5Lookup[instrument].Volatility / sumInverseVol
                            : 0.0;

                        int targetShares = (int)Math.Floor(weight * NetAssetValue[0] / instrument.Close[0]);
                        instrument.Trade(targetShares - instrument.Position);
                    }
                }

                if (TradingDays > 0 && !IsOptimizing)
                {
                    _plotter.AddNavAndBenchmark(this, benchmarkDs.Instrument);
                    _plotter.AddStrategyHoldings(this, universeDs.Select(u => u.Instrument));
                }
