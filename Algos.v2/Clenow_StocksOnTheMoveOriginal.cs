...
using static TuringTrader.Indicators.IndicatorsMomentum;
#endregion

namespace TuringTrader.BooksAndPubs
{
    public class Clenow_StocksOnTheMoveOriginal : AlgorithmPlusGlue
    {
        ...
        public override IEnumerable<Bar> Run(DateTime? startTime, DateTime? endTime)
        {
            ...
            WarmupStartTime = StartTime - TimeSpan.FromDays(256);
#endif
            ...
            foreach (DateTime simTime in SimTimes)
            {
                ...
                var constituents = Instruments.AsParallel().Where(i => i.IsConstituent(UNIVERSE))
                    .ToList();

                var indicatorsCals = new ConcurrentBag<(Instrument Instrument, _Regression Regression, ITimeSeries<double> Highest, ITimeSeries<double> SMA100, ITimeSeries<double> ATR20)>();
                Parallel.ForEach(Instruments, new ParallelOptions { MaxDegreeOfParallelism = Environment.ProcessorCount },
                    (i) =>
                    {
                        var regression = i.Close.LogRegression(MOM_PERIOD);
                        var maxMove = i.Close.SimpleMomentum(1).AbsValue().Highest(MOM_PERIOD);
                        var avg100 = i.Close.SMA(INSTR_TREND);
                        var atr20 = i.AverageTrueRange(ATR_PERIOD);

                        indicatorsCals.Add((i, regression, maxMove, avg100, atr20));

                    });
                var indicators = indicatorsCals
                  .ToDictionary(
                      i => i.Instrument,
                      i => new
                      {
